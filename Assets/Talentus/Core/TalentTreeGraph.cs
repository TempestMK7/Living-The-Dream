...
namespace com.cygnusprojects.TalentTree
{
	...
	public class TalentTreeGraph : ScriptableObject 
	{
                ...
                if (boughtTalents.Count > 0)
                {
                    ...
                    if (skillLevel.Level > 0 && skillLevel.Cost > 0)
                    {
                        ...
                        foreach (TalentTreeNodeBase item in boughtTalents)
                        {
                            ...
                            {
                                treeStillOk = false;
                                Debug.Log("tree not valid on talent " + item.name);
                                break;
                            }
                        }
...
#if UNITY_EDITOR
        public void UpdateGraphGUI(Event e, Rect viewRect, Rect origRect, GUISkin viewSkin, Color requiredTypeColor, Color optionalTypeColor)
        {
            if (connections.Count > 0)
            {
                for (int i = 0; i < connections.Count; i++)
                {
                    connections[i].UpdateConnectionGUI(e, viewRect, viewSkin, requiredTypeColor, optionalTypeColor);
                }
            }
            ...
            if (talents.Count > 0)
            {
                ...
                for (int i = 0; i < talents.Count; i++)
                {
                    talents[i].UpdateNodeGUI(e, viewRect, viewSkin);
                }
            }
        ...
        private void DrawConnectionToMouse(Vector2 mousePosition)
        {
            ...
            DrawNodeCurve(connectionNode.nodeRect, mousePosition);
        }
        
        private Vector3[] DrawNodeCurve(Rect start, Vector2 end)
        {
