...
namespace com.cygnusprojects.TalentTree
{
	public class TalentTreeNode : TalentTreeNodeBase
    {
        ...
        public override void UpdateNodeGUI(Event e, Rect viewRect, GUISkin viewSkin)
        {
            ...
            // Input
            if (GUI.Button(new Rect(nodeRect.x - HandleWidth, (nodeRect.y + (nodeRect.height - HandleHeight) / 2), HandleWidth, HandleHeight), "", viewSkin.GetStyle("NodeInput")))
            {
                if (parentTree != null)
                {
            ...
            // Output
            if (GUI.Button(new Rect(nodeRect.x + nodeRect.width, (nodeRect.y + (nodeRect.height - HandleHeight) / 2), HandleWidth, HandleHeight), "", viewSkin.GetStyle("NodeOutput")))
            {
                if (parentTree != null)
                {
